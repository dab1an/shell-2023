      ...
      <div className="w-10/12 flex flex-col items-center">
        {params.question === "result" ? (
          ...
          <>
            ...
            <div
              className="grid grid-cols-4 justify-evenly mt-14 gap-4 flex-wrap"
              style={{ flexWrap: "wrap" }}
            >
              {data.choices.map((choice, index) => {
                return (
                  <div
                    key={index}
                    className="flex bg-white border-2 border-neutral-900 w-56 rounded p-2 gap-2 h-12 items-center"
                  >
                    <p className="bg-[#30DF36] rounded-full w-6 h-6 flex justify-center items-center">
                      {index + 1}
                    </p>
                    <p>{choice}</p>
                  </div>
